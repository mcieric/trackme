...
export function AllocationChart({ balances }: AllocationChartProps) {
    ...
    const chainData = balances.reduce((acc: any[], curr) => {
        ...
        if (existing) {
            ...
            acc.push({
                name: curr.chainId === 1 ? 'Ethereum' :
                    curr.chainId === 8453 ? 'Base' :
                        curr.chainId === 10 ? 'Optimism' :
                            curr.chainId === 42161 ? 'Arbitrum' :
                                curr.chainId === 137 ? 'Polygon' : `Chain ${curr.chainId}`,
                value: curr.value,
    ...
    return (
        <Card className="h-[250px] w-full md:w-[350px] bg-zinc-900/50 border-zinc-800/50 backdrop-blur-sm flex flex-col relative">
            <div className="absolute top-4 left-4 z-10">
                <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Allocation</h3>
            </div>
                    ...
                    <Tooltip
                        content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                                return (
                                    <div className="bg-zinc-900/90 border border-zinc-700 p-2 rounded-lg shadow-xl backdrop-blur-md">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none pt-4">
                <div className="text-center">
                    <div className="text-xs text-zinc-500 font-medium">TOP</div>
